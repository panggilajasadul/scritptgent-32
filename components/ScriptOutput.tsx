...
const ScriptOutput: React.FC<ScriptOutputProps> = ({ killerTitle, variations, explanation, caption, hashtags, onSave }) => {
  ...
  const handleDownloadPdf = () => {
    ...
    // --- HEADER ---
    doc.setFontSize(14);
    doc.text('Generated Video Script ("Scriptgen 3.0")', doc.internal.pageSize.getWidth() / 2, y, { align: 'center' });
    y += 15;
    ...
    for (let i = 1; i <= pageCount; i++) {
        ...
        doc.text('aplikasi pembuat Skrip konten affiliasi SCRIPTGEN 3.0 bisa di order di shopee @smokervibes.id', doc.internal.pageSize.getWidth() / 2, pageHeight - 10, { align: 'center' });
    }
  ...
  const handleShare = () => {
    if (!activeScript) return;
    let fullText = `*Generated Video Script ("Scriptgen 3.0")*\n\n`;
    ...
    fullText += `_aplikasi pembuat Skrip konten affiliasi SCRIPTGEN 3.0 bisa di order di shopee @smokervibes.id_`;
